...
class BaiduPolyglot() : AbstractPolyglot(URL) {
    companion object {
        ...
        private const val AUTO_DETECT_URL = "https://fanyi.baidu.com/langdetect"
        private const val COOKIE_VALUE =
    ...
    override fun query(): String {
        if (isAutoDetected) {//自动检测源语言
            val request = HttpPost(AUTO_DETECT_URL)
...
//            println(entityStr)
            close(entity, response, false)
            val mapper = ObjectMapper()
            formData["from"] = mapper.readTree(entityStr).path("lan").asText()
        }
